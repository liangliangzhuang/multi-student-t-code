...
for(qqq in 1:itt){
  ...
  # NL_Wiener_R = R_cal(scen = "linear",para3 = em_re_nolinear_Wiener[[2]], r_t = rt_seq, r_SIG0 = em_re_nolinear_Wiener[[3]], B = B_item, yz = thr)[[1]][, 2]
  # NL_T_R = R_cal(scen = "linear",para3 = em_re_nolinear_T[[2]], r_t = rt_seq, r_SIG0 = em_re_nolinear_T[[3]], B = B_item, yz = thr)[[1]][, 2]
...
# save.image(file = paste("模拟实验/result/可靠度评估/I-n",n,"-m",m,".RData", sep = ""))
