...
ggsave("case/PMB/EM-iter/non-linear_Wiener-em.pdf", height = 3, width = 11)
...
for (h in 1:(item)) {
  # 1. 产生伪样本数据
  bt_dat <- sim_path(par = em_re_nolinear_Wiener[[2]], v = est_v, SIG0 = em_re_nolinear_Wiener[[3]], scen = "non-linear")
  y.diff_bt <- bt_dat[[1]]
  sumys_bt <- bt_dat[[2]]
