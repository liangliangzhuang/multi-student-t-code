...
para <- list("est_etas" = c(0.01,0.01,0.01)*100, "est_delta" = rep(1, p)*0.2, "est_sig0" = diag(p), "est_v" = 1)
...
f1_names = list(expression(hat(eta)[1]), expression(hat(eta)[2]), expression(hat(eta)[3]),
                expression(hat(delta)[1]), expression(hat(delta)[2]),expression(hat(delta)[3]),
                expression(hat(sigma)[1]), expression(hat(sigma)[2]), expression(hat(sigma)[3]), 
                expression(hat(rho)[12]), expression(hat(rho)[13]), expression(hat(rho)[23]), 
                expression(hat(nu)))

EM_iter_plot(para_iter = em_re_nolinear_T_no_na) # 绘制EM迭代图
# ggsave("case/crack/EM-iter/non-linear_T-em.pdf", height = 3, width = 11)
...
quan_nolinear_T <- data.frame(round(apply(bt_ci_T[bt_ci_T[,13] <80,], 2, quantile, c(0.05, 0.5, 0.95), na.rm = TRUE), 5))
